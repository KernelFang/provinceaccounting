(function ($) {
  ...
  function initializeCharacterCount() {
    // Loop through all textareas with class `character-count`
    $(".character-count").each(function () {
      let maxLength = $(this).attr("maxlength") || 255; // Default 255 if no maxlength specified
      let remaining = maxLength - $(this).val().length;


      // Set initial remaining characters
      let $remainingCharactersText = $(this).siblings('.form-feedback').find('.remaining-characters-text');
      let $remainingText = $(this).siblings('.form-feedback').find('.remaining-characters');
      $remainingText.text(remaining);

      // Apply red color if less than 50 characters are remaining
      if (remaining < 50) {
        $remainingText.addClass('text-danger');
        $remainingCharactersText.addClass('text-danger');
      } else {
        $remainingText.removeClass('text-danger');
        $remainingCharactersText.removeClass('text-danger');
      }
