<x-app-layout>
    ...
    <div class="row">
        <div class="col-xl-12">
            <form method="POST" action="{{ route('departments.update', $department) }}" class="form-style1">
                @csrf @method('PUT')

                <div class="ps-widget bgc-white bdrs4 p30 mb30 position-relative">
                    <div class="bdrb1 pb15 mb25">
                        <h5 class="list-title">{{ __('Update') . ' ' . __('Department') . ' ' . __('Details') }}</h5>
                    </div>
                    <div class="col-lg-10">
                        <div class="row">
                            @include('departments.form', ['item' => $department])
                        </div>
                    </div>
                </div>
