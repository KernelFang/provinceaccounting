<x-app-layout>
    <x-slot name="title">{{ __('Visa') . ' ' . __('Management') }}</x-slot>

    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center w-100">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Visa') . ' ' . __('Management') }}</h2>
            @canany(['visas.*', 'visas.create'])
                <a href="{{ route('visas.create') }}" class="ud-btn btn-light-thm py-1 px-3"><i
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            <div class="ps-widget bgc-white bdrs4 p30 mb30 position-relative">
                ...
                <form method="GET" action="{{ route('visas.index') }}" class="form-style1 compact g-3">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="mb20">
                                ...
                                <div class="switch-style1"
                                    ...
                                    <div class="form-check form-switch mb-0">
                                        <input class="form-check-input" type="checkbox" name="has_due" value="1"
                                            id="has_due_visas" {{ request()->filled('has_due') ? 'checked' : '' }}>
                                        <label class="form-check-label small text-muted" for="has_due_visas"
            ...
            @include('components.summary-cards', [
                'title' => 'Visas Summary',
                'subtitle' => 'Summary for displayed visa records.',
                'showing' => $visasTotal,
                'cards' => [
            ...
            <div class="ps-widget bgc-white bdrs4 p30 mb30 overflow-hidden position-relative">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="list-title mt-2">{{ __('Visas') }}</h5>

                    @canany(['visas.*', 'visas.restore', 'visas.force-delete'])
                        <div class="d-flex gap-2">
                            <a href="{{ route('visas.index') }}" class="btn btn-sm btn-outline-primary">All</a>
                            <a href="{{ route('visas.index', array_merge(request()->query(), ['with_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-secondary">With Trashed</a>
                            <a href="{{ route('visas.index', array_merge(request()->query(), ['only_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-danger">Only Trashed</a>
                        </div>
                    @endcanany
                </div>

                <p class="text-muted">A list of all visa bookings.</p>
                        ...
                        <tbody class="t-body">
                            ...
                            @foreach ($visas as $visa)
                                <tr class="text-center">
                                    <td>{{ ++$i }}</td>
                                    <td>
                                        {{ ucwords(strtolower($visa->purpose)) }}
                                    </td>
                                    <td>{{ $visa->country ?? 'N/A' }}</td>
                                    <td>{{ $visa->customer ?? 'N/A' }}</td>
                                    <td>{{ number_format($visa->customer_price, 2) }}</td>
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @canany(['visas.*', 'visas.show'])
                                                <a href="{{ route('visas.show', $visa->id) }}" class="icon me-2"
                                            ...
                                            @canany(['visas.*', 'visas.edit'])
                                                <a href="{{ route('visas.edit', $visa->id) }}" class="icon me-2"
                                            ...
                                            @canany(['visas.*', 'visas.destroy'])
                                                <form action="{{ route('visas.destroy', $visa->id) }}" method="POST"
                                                    ...
                                                    <a href="#" class="icon" data-bs-toggle="tooltip"
                                                        title="Delete" data-confirm data-confirm-title="Delete visa?"
                                            ...
                                            @if (method_exists($visa, 'trashed') && $visa->trashed())
                                                @canany(['visas.*', 'visas.restore', 'visas.force-delete'])
                                                    <x-delete-restore-buttons routePrefix="visas" :model="$visa" />
                                                @endcanany
                                            @endif
                                        </div>
                                    </td>
                    ...
                    @if (method_exists($visas, 'total'))
                        <div class="mbp_pagination mt30 text-center">
                            <ul class="page_navigation">
                                ...
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $visas->previousPageUrl() }}">
                                            <span class="fas fa-angle-left"></span>
                                        </a>
                                    </li>
                                @endif

                                @foreach ($visas->getUrlRange(1, $visas->lastPage()) as $page => $url)
                                    <li class="page-item {{ $visas->currentPage() == $page ? 'active' : '' }}">
                                ...
                                @if ($visas->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $visas->nextPageUrl() }}">
                            ...
                            <p class="mt10 mb-0 pagination_page_count text-center">
                                {{ $visas->firstItem() }} – {{ $visas->lastItem() }}
                                of {{ $visas->total() }} Records
                            </p>
                        </div>
                    @else
