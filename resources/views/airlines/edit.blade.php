<x-app-layout>
    <x-slot name="title">{{ __('Edit Airline') }}</x-slot>
    ...
    <div class="row">
        <div class="col-xl-12">
            <form method="post" action="{{ route('airlines.update', $item) }}" class="form-style1">
                {{ method_field('PATCH') }}
                @csrf

                <div class="ps-widget bgc-white bdrs4 p30 mb30 position-relative">
                    <div class="bdrb1 pb15 mb25">
                        <h5 class="list-title">{{ __('Update') . ' ' . __('Airline') . ' ' . __('Details') }}</h5>
                    </div>
                    <div class="col-lg-7">
                        <div class="row">
