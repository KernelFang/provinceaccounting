<x-app-layout>
    <x-slot name="title">{{ __('Purposes') }}</x-slot>

    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center w-100">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Purposes') }}</h2>
            @canany(['purposes.*', 'purposes.create'])
                <a href="{{ route('purposes.create') }}" class="ud-btn btn-light-thm py-1 px-3"><i
        ...
        <div class="col-xl-12">
            <div class="ps-widget bgc-white bdrs4 p30 mb30 overflow-hidden position-relative">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="list-title mt-2">{{ __('Purposes') }}</h5>

                    @canany(['purposes.*', 'purposes.restore', 'purposes.force-delete'])
                        <div class="d-flex gap-2">
                            <a href="{{ route('purposes.index') }}" class="btn btn-sm btn-outline-primary">All</a>
                            <a href="{{ route('purposes.index', array_merge(request()->query(), ['with_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-secondary">With Trashed</a>
                            <a href="{{ route('purposes.index', array_merge(request()->query(), ['only_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-danger">Only Trashed</a>
                        </div>
                    @endcanany
                </div>

                <p class="text-muted">A list of purposes.</p>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @canany(['purposes.*', 'purposes.edit'])
                                                <a href="{{ route('purposes.edit', $it) }}" class="icon me-2"><span
                                                        class="fas fa-pen"></span></a>
                                            @endcanany

                                            @canany(['purposes.*', 'purposes.destroy'])
                                                <form action="{{ route('purposes.destroy', $it) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @if (method_exists($it, 'trashed') && $it->trashed())
                                                @canany(['purposes.*', 'purposes.restore', 'purposes.force-delete'])
                                                    <x-delete-restore-buttons routePrefix="purposes" :model="$it" />
                                                @endcanany
                                            @endif
                                        </div>
                                    </td>
