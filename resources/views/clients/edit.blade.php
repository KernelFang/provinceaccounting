<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="">{{ __('Clients') . ' ' . __('Management') }}</h2>
    </x-slot>

    <div class="row">
        <div class="col-xl-12">
            <form method="post" action="{{ route('clients.update', $client->id) }}" class="form-style1">
                {{ method_field('PATCH') }}
                @csrf

                <div class="ps-widget bgc-white bdrs4 p30 mb30 position-relative">
                    <div class="bdrb1 pb15 mb25">
                        <h5 class="list-title">{{ __('Update') . ' ' . __('Client') . ' ' . __('Details') }}</h5>
                    </div>
                    <div class="col-lg-7">
