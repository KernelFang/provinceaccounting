<x-app-layout>
    <x-slot name="title">{{ __('Portals') }}</x-slot>

    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center w-100">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Portals') }}</h2>
            @canany(['portals.*', 'portals.create'])
                <a href="{{ route('portals.create') }}" class="ud-btn btn-light-thm py-1 px-3"><i
        ...
        <div class="col-xl-12">
            <div class="ps-widget bgc-white bdrs4 p30 mb30 overflow-hidden position-relative">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="list-title mt-2">{{ __('Portals') }}</h5>

                    @canany(['portals.*', 'portals.restore', 'portals.force-delete'])
                        <div class="d-flex gap-2">
                            <a href="{{ route('portals.index') }}" class="btn btn-sm btn-outline-primary">All</a>
                            <a href="{{ route('portals.index', array_merge(request()->query(), ['with_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-secondary">With Trashed</a>
                            <a href="{{ route('portals.index', array_merge(request()->query(), ['only_trashed' => 1])) }}"
                                class="btn btn-sm btn-outline-danger">Only Trashed</a>
                        </div>
                    @endcanany
                </div>

                <p class="text-muted">A list of all portals.</p>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @canany(['portals.*', 'portals.edit'])
                                                <a href="{{ route('portals.edit', $it) }}" class="icon me-2"
                                                    title="Edit"><span class="fas fa-pen"></span></a>
                                            @endcanany

                                            @canany(['portals.*', 'portals.destroy'])
                                                <form action="{{ route('portals.destroy', $it) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @if (method_exists($it, 'trashed') && $it->trashed())
                                                @canany(['portals.*', 'portals.restore', 'portals.force-delete'])
                                                    <x-delete-restore-buttons routePrefix="portals" :model="$it" />
                                                @endcanany
                                            @endif
                                        </div>
                                    </td>
